...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    keywordToReplace: '-',     // Converts, for example, {keyword}Origin to Origin before sending to destination URL
  };
  ...
  return function(req, res) {
    ...
    // Convert keyworded headers
    var keyword = corsAnywhere.keywordToReplace;
    for(var header in req.headers) {
      if (header.length > keyword.length) {
        if (header.substring(0, keyword.length) == keyword) {
          var headerWoKeyword = header.substring(keyword.length, header.length);
          req.headers[headerWoKeyword] = req.headers[header];
          delete req.headers[header];
        }
      }
    }
